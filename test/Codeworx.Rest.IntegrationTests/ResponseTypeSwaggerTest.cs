...
namespace Codeworx.Rest.UnitTests
{
    public class ResponseTypeSwaggerTest : TestServerTestsBase
    {
        ...
        [Fact]
        public async Task TestDownloadSwaggerFileHasResponseTypeMediaTypeForOKDefaultResponse()
        {
            var request = TestServer.CreateRequest("swagger/v1/swagger.json");
            var response = await request.SendAsync(HttpMethods.Get);
            ...
            var responses = paths.GetProperty("/api/StatusCode/Data").GetProperty("get").GetProperty("responses");
